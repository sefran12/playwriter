...
class GroqProvider(LLMProvider):
    ...
    async def complete(
        ...
        *,
        ...
        log.info("Groq complete: model=%s, json_mode=%s, prompt_len=%d",
        ...
        try:
            ...
            log.info("Groq response: %d chars", len(result))
            return result
    ...
    async def complete_structured(
        ...
        *,
        ...
        log.info("Groq structured: model=%s, target=%s",
        ...
        try:
            ...
            log.info("Groq structured parse OK: %s", response_model.__name__)
            return result
        except Exception as exc:
            log.error("Groq structured parse FAILED: %s — raw[:300]=%s",
                      exc, raw[:300])
            raise
